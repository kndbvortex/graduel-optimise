			  ...
			  int membership_retval, bool shared_tt)
{
	...
	// cout << "Pattern :" ;
	// for(element_t i_: pattern){
	// 	cout << i_ << ' ';
	// }
	// cout << endl;

	// cout << "Closure:";
	// for (element_t i_ : closed_pattern)
	// {
	// 	cout << i_ << ' ';
	// }
	// cout << endl;
	...
	// cout  << "#####" << (float)closed_pattern.size() / (float)membership_retval << ".....";
	// if(closed_pattern.size() > 1)
	...
	if (augmentations.size() > 0)
	{
		...
		else
		{
			...
			if (new_shared_tt)
				increase_nb_refs(&tt, augmentations.size() - 1);
...
void paraminer_usage(char *bin_name, bool exit)
{
	...
	cerr << bin_name << " [-t <numthreads> = 1] [-c <tuplecutoff> = 2] " << endl
		 ...
		 << "<problem specific command line arg 1> .. <problem specific command line arg n>" << endl
		 << endl;
...
#else
	cout << "empty set before clo " <<endl;
