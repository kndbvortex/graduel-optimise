...
BoolMatrix deleteRowAndColumn(const BoolMatrix &matrix, int i)
{
	...
	BoolMatrix result(N - 1);
	std::vector<std::string> columnNames = matrix.get_column_names();
	columnNames.erase(std::find(columnNames.begin(), columnNames.end(), columnNames[i]));
	result.set_column_names(columnNames);
	...
	for (int row = 0; row < N; ++row)
	{
		...
		for (int col = 0; col < N; ++col)
		{
			if (col == i)
				continue;
			result.set_value(newCol, newRow, matrix.get_value(col, row));
			++newCol;
		}
		++newRow;
	}
...
int sumRowAndColumn(const BoolMatrix &matrix, int i)
{
	int N = matrix.get_size();
	if (i < 0 || i >= N)
	{
	...
	for (int col = 0; col < N; ++col)
	{
		sum += matrix.get_value(col, i) ? 1 : 0;
	}
	...
	for (int row = 0; row < N; ++row)
	{
		if (row != i)
		{
			sum += matrix.get_value(i, row) ? 1 : 0;
		}
	}
...
{
	// cout << "before assert: taille de la matrice " << bm->get_size() << " nb trans " << nb_trans << " threshold :" << threshold << endl;
	...
	// printMatrix(*bm, bm->get_size(), bm->get_size());
	// for(int i=0; i<bm->get_size(); i++){
	// 	if(sumRowAndColumn(*bm, i) < threshold){
	// 		bm->deleteRowAndColumn(i);
	// 		i--;
	// 	}
	// }
}
...
void print_grad_transaction_table(const TransactionTable &tt)
{
	for (int i = 0; i < tt.size(); i++)
	{
		...
		cout << i << " " << t.first_ << "x" << t.second_ << " (" << tt[i].weight << ") : ";
		print_grad_transaction(tt[i]);
		cout << endl;
	}
}
...
void read_transaction_table_vtrans(TransactionTable *tt, const char *filename)
{
	...
	while (ifs.good())
	{
		...
		while (ss.good())
		{
			if (item >= (numeric_limits<double>::max()) / 1000)
			{
				cerr << "No value larger than " << numeric_limits<float>::max() / 1000 << " allowed in input file (See. "
...
{
	cout << input.size() * input.size() - input.size() << "à reserver" << endl;
	output->reserve(input.size() * input.size() - input.size());
	cout << "Reservation effectué" << endl;
	int nb_trans = 0;
...
{
	cout << " " << (int)(element / 2) + 1 << ((int)element % 2 ? "-" : "+");
}
					  ...
					  const membership_data_t &data)
{
	...
	if ((int)(s[0]) % 2 == 1) /* remove from F sets whose first element is a X- */
		return 0;
	if (base_set.size() == 0)
		nb_vtrans;
...
for(int i = 0; i < s.size()-1; i++){
	...
	// }
	// cout << nb_max_tids.first << " .....////" << endl;
	BoolMatrix bm(nb_max_tids.first);
	// cout << nb_max_tids.first << " nb_max_tids.first" << endl;

	/* Rename tids in the range [0, max_tids] */
	vector<int16_t>
		back_perms(nb_max_tids.second + 1, -1);
	int perm_idx = 0;
	...
	int sup = 0;
	// cout<< threshold << "...." << s_ << "####" << bm.get_size() <<  endl;

	// cout << endl;
	// cout << "'Path length: " << endl;
	// for (auto e: path_length){
	// 	cout << e << " ";
	// }
	// cout << endl;

	sup = compute_gradual_support(bm, &path_length);
	// cout << "'Path length après calcul du support: " << endl;
	// for (auto e : path_length)
	// {
	// 	cout << e << " ";
	// }
	// cout << endl;
	// cout << "Matrice binaire " << endl;
	// printMatrix(bm);
	// exit(EXIT_FAILURE);
	// if (s_ >= (float)(threshold * (threshold - 1)) / 2.0)
	// {
	// }
	...
	// 	{
	// 		cout << bm[i];
	// 		if (i % bm.get_size() == bm.get_size() - 1)
	// 		{
	// 			cout << endl;
	// 		}
	// 		else
...
set_t clo(const set_t &set, const closure_data_t &data)
{
	...
	// cout << "Size before: " << nb_vtrans << endl;

	fill_bool_matrix(&bm, transaction_pairs, nb_vtrans);
	// printMatrix(bm, bm.get_size(), bm.get_size());

	// cout << "Size of the matrix " << bm.size() << endl;
	// for (auto i : bm)
	// 	std::cout << i << ' ';
	// cout << endl;
	// cout << "Nombre de transactions " << nb_initial_trans << endl;
...
int main(int argc, char **argv)
{
	...
	// cout << "Nombre de parametre " << argc << endl;
	...
	cout << "La table en question" << endl;
